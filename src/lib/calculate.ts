...
export function calculateWAM(results: Result[]): number {
  ...
  const { weightedMarksSum, weightedCreditPointsSum } = results.reduce(
    (acc, unit) => {
      ...
      return {
        ...
        weightedCreditPointsSum:
          acc.weightedCreditPointsSum + weightedCreditPoints,
      };
    },
...
export function calculateGPA(results: Result[]): number {
  ...
  const { weightedGPASum, totalCreditPoints } = results.reduce(
    (acc, unit) => {
      // Skip excluded grades
      if (!(unit.grade in CORE_GRADES)) {
        return acc;
      ...
      const gradeValue =
        CORE_GRADES[unit.grade as keyof typeof CORE_GRADES].gpaValue;
